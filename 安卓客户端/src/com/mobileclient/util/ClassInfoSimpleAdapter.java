...
/*需要绑定的数据*/
  ...
  public View getView(int position, View convertView, ViewGroup parent) { 
	  ...
	  holder.tv_specialName.setText("所在专业：" + mData.get(position).get("specialName").toString());
	  holder.tv_className.setText("班级名称：" + mData.get(position).get("className").toString());
	  try {holder.tv_startDate.setText("成立日期：" + mData.get(position).get("startDate").toString().substring(0, 10));} catch(Exception ex){}
	  /*返回修改好的view*/
	  return convertView; 
    } 
