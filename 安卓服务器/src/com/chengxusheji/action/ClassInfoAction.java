...
public class ClassInfoAction extends BaseAction {
    ...
    public String QueryClassInfo() {
        ...
        List<ClassInfo> classInfoList = classInfoDAO.QueryClassInfoInfo(classNumber, specialName, className, startDate, currentPage);
        /*计算总的页数和总的记录数*/
        classInfoDAO.CalculateTotalPageAndRecordNumber(classNumber, specialName, className, startDate);
        /*获取到总的页码数目*/
    ...
    public String QueryClassInfoOutputToExcel() { 
        ...
        if(className == null) className = "";
        if(startDate == null) startDate = "";
        List<ClassInfo> classInfoList = classInfoDAO.QueryClassInfoInfo(classNumber,specialName,className,startDate);
        ExportExcelUtil ex = new ExportExcelUtil();
        String title = "ClassInfo信息记录"; 
        String[] headers = { "班级编号","所在专业","班级名称","成立日期","班主任"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<classInfoList.size();i++) {
        	ClassInfo classInfo = classInfoList.get(i); 
        	dataset.add(new String[]{classInfo.getClassNumber(),classInfo.getSpecialName(),classInfo.getClassName(),new SimpleDateFormat("yyyy-MM-dd").format(classInfo.getStartDate()),classInfo.getHeadTeacher()});
        }
        /*
